...
Create posts in `content/writing/articles/` or `content/writing/stories/`:
...
`src/lib/seo.ts` is the shared source for the canonical origin, professional and full names, portrait, and public profile links. The canonical site is `https://www.lokmanefe.com`; preview deployments also point to this origin. The local portrait in `public/images/lokman-baturay-efe.jpg` is the existing LinkedIn profile photo.

Writing and book frontmatter accepts an optional `lastModified: "2026-09-08"` (ISO date or timestamp). Set it only when the content materially changes. Without it, sitemap `lastmod`, article `dateModified`, and social modification dates are omitted. Filesystem timestamps are never used for these fields. RSS omits a book's publication date when no explicit full reading/update date is available.

Book pages stay statically generated and indexable, including unrated books; only rated books emit Review markup. Reading and Writing archives render their URL filters on the server. `listed: false` hides a published writing from archives, RSS, and the sitemap, while preserving direct access and indexing; `published: false` makes it unavailable.

Page-specific share images are generated by `/og?title=…`. Domain redirects must also be configured in Vercel so TLS and routing work before requests reach Next.js. After publishing, submit `/sitemap.xml` in Search Console, inspect representative URLs, and compare the two name queries over at least 28 days. A new public PageSpeed run must use the deployed version.
